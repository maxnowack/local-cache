...
beforeAll(() => {
  ...
  jest.spyOn(core, 'setOutput').mockImplementation((key, value) =>
    jest.requireActual('@actions/core').setOutput(key, value))
})
...
test('StateProvider saves states', () => {
  ...
  const saveStateMock = jest
    ...
    .mockImplementation((key, value: string) => {
      states.set(key, value)
    })

  const setOutputMock = jest
    .spyOn(core, 'setOutput')
    .mockImplementation((key, value) => jest.requireActual('@actions/core').setOutput(key, value))
...
test('NullStateProvider saves outputs', () => {
  const getStateMock = jest
    .spyOn(core, 'getState')
  ...
  const saveStateMock = jest
    .spyOn(core, 'saveState')
    .mockImplementation((key, value) => jest.requireActual('@actions/core').saveState(key, value))
